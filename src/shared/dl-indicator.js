...
(function (global) {
  ...
  function ringSVG(indeterminate, size = 28) {
    ...
    return (
      '<svg class="operant-dl-ring" viewBox="0 0 ' + size + " " + size + '" width="' + size + '" height="' + size + '" focusable="false" aria-hidden="true">' +
      '<circle class="operant-dl-ring-track" cx="' + size / 2 + '" cy="' + size / 2 + '" r="' + r + '" fill="none" stroke-width="2"/>' +
      '<circle class="operant-dl-ring-bar" cx="' + size / 2 + '" cy="' + size / 2 + '" r="' + r + '" fill="none" stroke-width="2"' +
  ...
  function checkSVG() {
    return (
      '<svg class="operant-dl-check" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" focusable="false" aria-hidden="true">' +
  ...
  function errorSVG() {
    return (
      '<svg class="operant-dl-err" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" focusable="false" aria-hidden="true">' +
  ...
  // variables con fallback: --operant-accent (morado), --operant-success (verde),
  // --operant-danger (rojo), --operant-text-secondary.
  function dlIndicatorCSS() {
    ...
    .operant-dl {
      ...
      width: 28px;
      height: 28px;
      color: var(--operant-text-secondary, #94a3b8);
    }
    /* Variante compacta (overlay de página): botón de 24px con anillo de 22px. */
    .operant-dl.operant-dl-sm {
      width: 24px;
      height: 24px;
    }
    .operant-dl.operant-dl-sm .operant-dl-ring {
      width: 22px;
      height: 22px;
    }
    .operant-dl.operant-dl-sm .operant-dl-idle svg {
      width: 18px;
      height: 18px;
    }
    .operant-dl .operant-dl-idle svg {
      display: block;
      width: 20px;
      height: 20px;
    }
    .operant-dl .operant-dl-stage,
    .operant-dl .operant-dl-check,
    .operant-dl .operant-dl-err {
      position: absolute;
      inset: 0;
      display: flex;
      align-items: center;
    ...
    .operant-dl .operant-dl-ring {
      display: block;
    }
    .operant-dl .operant-dl-ring-track {
      stroke: var(--operant-accent, currentColor);
      stroke-opacity: 0.15;
    }
    .operant-dl .operant-dl-ring-bar {
      stroke: var(--operant-accent, currentColor);
      will-change: stroke-dashoffset;
    }
    .operant-dl .operant-dl-check {
      color: var(--operant-success, #22c55e);
      pointer-events: none;
      will-change: opacity;
    }
    .operant-dl .operant-dl-check path {
      will-change: stroke-dashoffset;
    }
    .operant-dl .operant-dl-err {
      color: var(--operant-danger, #ef4444);
      pointer-events: none;
      will-change: opacity;
    }
    .operant-dl.operant-dl-error .operant-dl-ring-bar,
    .operant-dl.operant-dl-error .operant-dl-ring-track {
      stroke: var(--operant-danger, #ef4444);
    }
    .operant-dl.operant-dl-error .operant-dl-ring-track {
      stroke-opacity: 0.15;
    }
    /* Indeterminado: el anillo rota completo (360deg en 1.2s, linear). */
    @keyframes operant-dl-spin {
      0% { transform: rotate(0deg); }
      100% { transform: rotate(360deg); }
    }
    .operant-dl.operant-dl-progress .operant-dl-ring-bar[data-indet="1"] {
  ...
  class DLIndicator {
    ...
    constructor(host, opts = {}) {
      ...
      host.classList.add("operant-dl");
      if (opts.size === "sm") host.classList.add("operant-dl-sm");
    ...
    _renderIdle() {
      this.state = "idle";
      this.host.classList.remove("operant-dl-progress", "operant-dl-success", "operant-dl-error");
      this.host.innerHTML = "";
      this._idleEl = document.createElement("span");
      this._idleEl.className = "operant-dl-idle";
    ...
    setProgress(p) {
      ...
      this.state = "progress";
      this.host.classList.add("operant-dl-progress");
      this.host.classList.remove("operant-dl-success", "operant-dl-error");
      // Re-render solo si no estamos ya en progress (evitar parpadeo del DOM).
      if (!this._ringBar) {
        this.host.innerHTML = "";
        const wrap = document.createElement("span");
        wrap.className = "operant-dl-stage";
        const size = this.host.classList.contains("operant-dl-sm") ? 22 : 28;
        wrap.innerHTML = ringSVG(indet, size);
        this.host.appendChild(wrap);
        this._ringBar = wrap.querySelector(".operant-dl-ring-bar");
        this._ringTrack = wrap.querySelector(".operant-dl-ring-track");
      ...
      if (indet) {
        ...
        this._ringBar.style.animation = "operant-dl-spin 1.2s linear infinite";
      } else {
    ...
    success() {
      ...
      this.state = "success";
      this.host.classList.add("operant-dl-success");
      this.host.classList.remove("operant-dl-progress", "operant-dl-error");
      // Mantener el anillo completo visible; añadir el check DENTRO.
      if (!this._checkEl) {
        this._checkEl = document.createElement("span");
        this._checkEl.className = "operant-dl-check";
    ...
    error() {
      this._clear();
      this.state = "error";
      this.host.classList.add("operant-dl-error");
      this.host.classList.remove("operant-dl-progress", "operant-dl-success");
      if (!this._ringBar) {
        this.host.innerHTML = "";
        const wrap = document.createElement("span");
        wrap.className = "operant-dl-stage";
      ...
      if (!this._errEl) {
        ...
        this._errEl.className = "operant-dl-err";
    ...
    _afterDone() {
      ...
      if (this._hovered || this.host.matches(":hover")) {
        ...
        this.host.dispatchEvent(new CustomEvent("operant-dl-idle", { detail: { keepHover: false } }));
    ...
    _backToIdle() {
      ...
      idle.className = "operant-dl-idle";
      ...
      this.state = "idle";
      this.host.classList.remove("operant-dl-progress", "operant-dl-success", "operant-dl-error");
      this.host.dispatchEvent(new CustomEvent("operant-dl-idle", { detail: { keepHover: true } }));
    }
  ...
  global.OperantDLIndicator = DLIndicator;
  global.OperantDLIndicatorCSS = dlIndicatorCSS;
